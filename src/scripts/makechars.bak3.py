        ...
        for parent in sorted(all_poses, key=lambda p: len(p.expression_t)):
            ...
            for subpose in parent.children:
                ...
                if new_patch:
                    ...
                    # parent.children.remove(subpose)
                    # parent.children.append(new_patch)
                    # for subpose2 in subpose.children:
                    #     subpose2.parent = new_patch
...
def main():
    ...
    for sprite_dir in args.character_dirs:
        logger.info(f"Loading poses from directory '{sprite_dir}'")
        all_poses = [
    ...
    for yaml_path in args.character_defs:
        logger.info(f"Loading poses from file '{yaml_path}'")
