...
class Pose():
    ...
    def onParentCropped(self, side_space, top_crop):
        # We're a base pose, we do not care.
        return

    def spriteTrim(self):
        sprite_image = self.getImage()

        cropped, side_space, top_crop = imageTrim(sprite_image)
        if cropped == sprite_image:
            logger.debug(f"{self} already cropped at {sprite_image.getbbox()}")
            return

        logger.debug(f"Cropping {self} from {sprite_image.getbbox()} to {cropped.getbbox()}")
        self.setImage(cropped)
        for pose in self.children:
            pose.onParentCropped(side_space, top_crop)
...
class PatchPose(Pose):
    ...
    def patchIsEfficient(self):
        ...
        replace_size = operator.mul(*imageTrim(self.full.getImage())[0].size)
    ...
    def onParentCropped(self, side_space, top_crop):
        logger.debug(f"{self} parent cropped! {side_space=}, {top_crop=}")
        ...
        # super().onParentCropped(side_space, top_crop)
...
def processPoses(all_poses, trim, patch, rpy_outpath, make_demo=False):
    ...
    if patch:
        ...
        all_poses_from_tree = getAllPosesFromTree()
        max_size_canvas = Pose("canvas", []).setImage(Image.new("1", (1280, 720)))
