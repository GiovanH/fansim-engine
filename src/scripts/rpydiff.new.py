...
def spriteLocation(char_name):
    ...
    """
    return "{{assets}}/sprites/" + char_name + "/"
...
class Pose():
    ...
    def __init__(self, from_path=None, from_dict=None):
        """Pass data from json
        """
        if from_path:
        elif from_dict:
        else:
            raise NotImplementedError("Must provide source for pose!")
        # try:
...
def getPatch(original_pose, update_pose, mode="box", fuzz=None, box_pad=2):
    ...
    #     try:
    #         mask = ImageChops.difference(original, update).convert('1')
    #     except ValueError:
    #         import traceback
    #         traceback.print_exc()
...
def doDiff(char_name, base_pose, updated_pose, rpyfp, char_options={}):
    """Summary
...
def parsePathToPoses(pngpath):
    print(pngpath)
    a, b = os.path.split(pngpath)
    char_name = os.path.split(a)[1]
    file_plainname, file_ext = os.path.splitext(b)
    subposes = re.split(r"[ -_]", file_plainname)

    print(f"{char_name=}, {subposes=}")
    return (char_name, subposes)


def generateConfigFromDir(sprite_dir, save_auto_yaml=False):
    print(sprite_dir)
    data = {sprite_dir: {"options": {}, "patchsets": {}}}

    for png in glob.glob(os.path.join(sprite_dir, "*.png")):
        (char_name, subposes) = parsePathToPoses(png)

    if save_auto_yaml:
        sprite_dir_name = os.path.split(sprite_dir)[1]
        with open(sprite_dir_name + ".yaml", "w") as fp:
...
def main():
    ...
    ap.add_argument(
        ...
        help="Remove whitespace from around sprites")
    ap.add_argument(
        "--save-auto-yaml", action="store_true",
        help="Convert character definitions into yaml files")
    ap.add_argument(
        "--patch", action="store_true",
        help="Generate patches to compress character sprites")
    args = ap.parse_args()

    character_defs = []

    for sprite_dir in args.character_dirs:
        data = generateConfigFromDir(sprite_dir, save_auto_yaml=args.save_auto_yaml)
        character_defs.append(data)

    for yaml_path in args.character_yaml:
            ...
            with open(os.path.join(outdir, char_name + ".rpy"), "w") as rpyfp:
                ...
                for patchdata in char_patches:
                    ...
                    for updated_pose in updated_poses:
                        doDiff(char_name, base_pose, updated_pose, rpyfp, char_options)
