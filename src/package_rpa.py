...
for package_id in sys.argv[1:]:
    ...
    pysubdir_new = os.path.join(fse_root, f"custom_volumes/{package_id}_min/")
    ...
    rpapath = os.path.join(fse_root, f"custom_volumes/{package_id}_min/assets.rpa")
    cmdstr = f"python {rpatool} -c {rpapath}"
